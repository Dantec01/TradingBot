...
async function run(config) {
    ...
    // eliteTickOffset: Percentage offset for Breakeven+Tick SL (default 0.1 = 0.1%)
    // IMPORTANT: This is a PERCENTAGE, same as Paper/Real bots
    const eliteTickOffset = parseFloat(config.eliteTickOffset) || 0.1;
    ...
    for (let i = 0; i < backtestCandles.length - 1; i++) {
        ...
        if (position) {
            ...
            if (closeReason) {
                ...
                } else if (strategy === 'SPIRIT_TEST') {
                    // SPIRIT_TEST: Immediate re-entry at exit price (ece2c11 behavior)
                    // Does NOT wait for signal confirmation - places limit order immediately
                    pendingLimit = {
                        type: position.type, // Same direction as closed position
                        price: exitPrice,    // Limit at the exit (SL hit) price
                        entryTime: nextCandle.time,
                        margin: Math.min(orderSize, balance),
                        initialSL: 0,
                        isTest: true         // Flag for SPIRIT_TEST
                    };
                }
            }
        }
            ...
            if (filled) {
                ...
                if (balance >= (margin + entryFee)) {
                    ...
                    position = {
                        ...
                        // SPIRIT_TEST: Set Elite flags for hybrid SL logic
                        isElite: pendingLimit.isTest || false,
                        isTest: pendingLimit.isTest || false
                    };
        ...
        if (!position && !pendingLimit) {
            ...
            if (candle.isTrigger && ((candle.signalStr === 'Buy' && canLong) || (candle.signalStr === 'Sell' && canShort))) {
                ...
                if (strategy === 'BACKGUARD') {
                ...
                } else if (strategy === 'SPIRIT_ELITE' || strategy === 'SPIRIT_TEST') {
                    // SPIRIT_ELITE / SPIRIT_TEST: Entrada Limit Agresiva (Simulada como Market al Cierre)
                    ...
                    // SPIRIT_TEST: Same entry logic as SPIRIT_ELITE
                    ...
                    if (balance >= (tradeAmount + entryFee)) {
                        ...
                        position = {
                            ...
                            isElite: true, // Flag for special Elite SL logic (Experimental + Tick + Trailing)
                            isTest: strategy === 'SPIRIT_TEST', // Flag for SPIRIT_TEST variant
                            candlesSinceEntry: 0,
                    ...
                    if (strategy === 'CLOSE_ENTRY' || strategy === 'SPIRIT' || strategy === 'SPIRIT_IMPROVED' || strategy === 'SPIRIT_TRAILING' || strategy === 'SPIRIT_EXPERIMENTAL' || strategy === 'SPIRIT_EXPERIMENTAL_FIXED') {
        ...
        if (position && position.stopActiveFromIndex !== undefined && position.stopActiveFromIndex <= (i + 1)) {
            ...
            if (position.type === 'LONG') {
                position.highestPrice = Math.max(position.highestPrice || position.entryPrice, candle.high); // candle is the SIGNAL candle (i). nextCandle is future. 
                // Wait, logic evaluates i+1 (nextCandle) for hit. So we should update extremes with i?
                // Actually backtest loop: i is confirmed. nextCandle is the one we check for hits.
                // So strict correctness: update extreme with nextCandle AFTER check? 
                // But for SL calculation (effectiveSL), we use data available up to 'i'.
                // 'nextCandle' is what we are testing against.
            } else {
                position.lowestPrice = Math.min(position.lowestPrice || position.entryPrice, candle.low);
            }

            // For BACKGUARD, SPIRIT_ELITE & SPIRIT_TEST: Update candlesSinceEntry
            if (strategy === 'BACKGUARD' || strategy === 'SPIRIT_ELITE' || strategy === 'SPIRIT_TEST') {
            ...
            if (strategy === 'BACKGUARD') {
                ...
                // 2. Elite Breakeven (Entry +/- % offset) - Activates next candle
                // eliteTickOffset is a PERCENTAGE (e.g., 0.1 = 0.1%)
                // Convert to price offset: entryPrice * (eliteTickOffset / 100)
                let eliteBESL = 0;

                // Activate immediately after entry candle (next candle start) == candlesSinceEntry >= 1
                if (position.candlesSinceEntry >= 1) {
                    const offsetVal = position.entryPrice * (eliteTickOffset / 100);
                    if (position.type === 'LONG') {
            ...
            if (position.type === 'LONG') {
                ...
                if ((strategy === 'SPIRIT_ELITE' || strategy === 'SPIRIT_TEST') && effectiveSL > 0 && nextCandle.low <= effectiveSL) {
                ...
                if ((strategy === 'SPIRIT_ELITE' || strategy === 'SPIRIT_TEST') && effectiveSL > 0) {
                    // For SHORT with SL below entry (profit lock):
                    // Price must go DOWN to SL level AND come back UP through it
                    // This prevents false positives from lookahead bias
                    if ((strategy === 'SPIRIT_ELITE' || strategy === 'SPIRIT_TEST') && effectiveSL > 0 && nextCandle.high >= effectiveSL) {
                        closeReason = `Stop Loss (Elite: ${position.eliteSLType})`;
                        exitPrice = effectiveSL;
                    }
                }
            ...
            if (closeReason) {
                ...
                const canReEnter = isSpirit && (isStopLoss || (isTrailing && strategy === 'SPIRIT_TRAILING'));
                    ...
                    if (i + 2 < backtestCandles.length) {
                        ...
                        if (checkCandle.signalStr === neededSignal) {
                            ...
                            if (strategy === 'SPIRIT_ELITE') {
                                // SPIRIT_ELITE: Immediate Aggressive Re-entry
                                // Price = Close of the candle where signal appeared (checkCandle.close)
