...
async function run(config) {
    ...
    for (let i = 0; i < backtestCandles.length - 1; i++) {
        ...
        let reversalEntry = false; // Flag: entry after Signal Reversal uses nextCandle.open
        ...
        // --- 3.1 Manage EXIT ---
        // Exit by signal reversal is executed at candle.close (same candle where opposite signal detected).
        ...
        if (position) {
            ...
            if (closeReason) {
                ...
                if (closeReason === 'Signal Reversal') {
                    pendingLimit = null;
                    reversalEntry = true; // New entry will use nextCandle.open
        ...
        if (!position && !pendingLimit) {
            ...
            if (candle.isTrigger && ((candle.signalStr === 'Buy' && canLong) || (candle.signalStr === 'Sell' && canShort))) {
                ...
                if (strategy === 'BACKGUARD') {
                    ...
                    if (reversalEntry) {
                        // Signal Reversal: enter at the open of the next candle
                        entryPrice = nextCandle.open;
                        entryTime = nextCandle.time;
                        stopActiveFromIndex = i + 2;
                    } else if (strategy === 'CLOSE_ENTRY' || strategy === 'SPIRIT' || strategy === 'SPIRIT_IMPROVED' || strategy === 'SPIRIT_TRAILING' || strategy === 'SPIRIT_EXPERIMENTAL' || strategy === 'SPIRIT_EXPERIMENTAL_FIXED') {
