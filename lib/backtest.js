...
async function run(config) {
    ...
    for (let i = 0; i < backtestCandles.length - 1; i++) {
        ...
        if (!position && !pendingLimit) {
            ...
            if (candle.isTrigger && ((candle.signalStr === 'Buy' && canLong) || (candle.signalStr === 'Sell' && canShort))) {
                ...
                if (strategy === 'BACKGUARD') {
                    ...
                    // SPIRIT_ELITE: Limit Order Entry + 5-Candle Lookback SL
                    const lookback = 5;
                    const startIdx = Math.max(0, i - lookback + 1);
                    const relevantCandles = backtestCandles.slice(startIdx, i + 1);

                    let eliteSL = 0;
                    if (candle.signalStr === 'Buy') {
                        eliteSL = Math.min(...relevantCandles.map(c => c.low));
                    } else {
                        eliteSL = Math.max(...relevantCandles.map(c => c.high));
                    }
                    ...
                    pendingLimit = {
                        ...
                        margin: tradeAmount,
                        initialSL: eliteSL,
                        isElite: true // Flag for special SL handling
                    };
                } else {
                    ...
                    if (balance >= (tradeAmount + entryFee)) {
                        ...
                        // notional is already calculated
        ...
        if (position && position.stopActiveFromIndex !== undefined && position.stopActiveFromIndex <= (i + 1)) {
            ...
            // For BACKGUARD: Update candlesSinceEntry
            if (strategy === 'BACKGUARD') {
            ...
            if (strategy === 'BACKGUARD') {
                ...
                // Activate immediately after entry candle (next candle start)
                if (nextCandle.time > position.entryTime) {
                    if (position.type === 'LONG') {
                ...
                // 4. Delayed Trailing Stop (after 5 candles)
                if (position.candlesSinceEntry >= 5) {
            ...
            if (position.type === 'LONG') {
                ...
                if (strategy === 'SPIRIT_ELITE' && position.eliteSLActivated && effectiveSL > 0) {
                ...
                if (canReEnter) {
                    ...
                    if (i + 2 < backtestCandles.length) {
                        ...
                        if (checkCandle.signalStr === neededSignal) {
                            ...
                            // SPIRIT_ELITE Re-entry: Use Limit Order
                            if (strategy === 'SPIRIT_ELITE') {
                                pendingLimit = {
                                    type: lastType,
                                    price: exitPrice, // Limit at Exit Price
                                    entryTime: entryCandle.time,
                                    margin: tradeAmount,
                                    initialSL: 0, // Elite calculates its own SL later
                                    isElite: true
                                };
                            } else {
                                // Standard SPIRIT Re-entry (Market at Exit Price)
                                const entryFee = tradeAmount * feeRate;
                                if (balance >= (tradeAmount + entryFee)) {
                                    balance -= (tradeAmount + entryFee);
                                    // Notional based on leverage
                                    const notionalEntry = tradeAmount * leverage;
                                    const size = notionalEntry / exitPrice;
