...
async function run(config) {
    ...
    const stopAtEntry = config.stopAtEntry === true || config.stopAtEntry === 'true';
    ...
    for (let i = 0; i < backtestCandles.length - 1; i++) {
        ...
        if (!position) {
            ...
            if (strategy === 'CLOSE_ENTRY' || strategy === 'SPIRIT' || strategy === 'SPIRIT_IMPROVED') {
        ...
        if (position && position.stopActiveFromIndex !== undefined && position.stopActiveFromIndex <= (i + 1)) {
            ...
            if (position.type === 'LONG') {
                const slPrice = position.entryPrice * (1 - slRate);
                if (stopAtEntry && nextCandle.low <= position.entryPrice) {
                    console.log(`[DEBUG SL] Time: ${new Date(nextCandle.time).toISOString()} | Type: LONG | Low: ${nextCandle.low} <= Entry: ${position.entryPrice} ? TRUE`);
                    closeReason = 'Stop Loss (Breakeven)';
                    exitPrice = position.entryPrice;
                } else if (!stopAtEntry && slRate > 0 && nextCandle.low <= slPrice) {
                    closeReason = 'Stop Loss';
                    exitPrice = slPrice;
                }
            } else if (position.type === 'SHORT') {
                const slPrice = position.entryPrice * (1 + slRate);
                if (stopAtEntry && nextCandle.high >= position.entryPrice) {
                    closeReason = 'Stop Loss (Breakeven)';
                    exitPrice = position.entryPrice;
                } else if (!stopAtEntry && slRate > 0 && nextCandle.high >= slPrice) {
                    closeReason = 'Stop Loss';
                    exitPrice = slPrice;
                }
            }
            ...
            if (closeReason) {
                ...
                if (closeReason && closeReason.startsWith('Stop Loss')) {
                    const neededSignal = (lastType === 'LONG') ? 'Buy' : 'Sell';

                    // 1. Standard SPIRIT: Immediate Re-entry
                    if (strategy === 'SPIRIT') {
