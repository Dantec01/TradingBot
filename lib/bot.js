...
class TradingEngine {
            ...
            if (isTrigger || isSpiritReEntry) {
                ...
                if (canEnter) {
                    if (this.strategy === 'BACKGUARD') {
                        // BACKGUARD: Place Virtual Limit Order at Close Price
                        this.pendingLimitOrder = {
                            type,
                            price: candle.close,
                            time: candle.time
                        };
                        console.log(`[${this.symbol}] BACKGUARD - Limit Order Placed at ${candle.close} (${type})`);
                    } else {
    ...
    manageOpenPosition(tick) {
        ...
        // SPIRIT_ELITE v4 (Paper): Experimental + Breakeven + Delayed Trailing
        if (this.strategy === 'SPIRIT_ELITE') {
            const eliteTickOffset = this.eliteTickOffset || 0.0001; // 1 tick default
            const trailingDelay = 5; // Candles before trailing activates

            // 1. Experimental SL Logic (Dynamic)
            if (!p.experimentalSLSet && tick.time > p.entryTime) {
                // In Paper, we use tick.close as candle close estimate if updated per candle
                // OR we rely on updateCandle() calling this logic.
                // Assuming this is called ON CANDLE UPDATE (new candle start)
                const lastCandle = this.candles[this.candles.length - 2];
                // Wait, updatePosition is called on EVERY TICK. 
                // We need to know if a candle just closed.
                // Simplified: Set Exp SL if we have a candle AFTER entry time
                const lastClosedCandle = this.candles[this.candles.length - 1]; // Latest closed candle
                if (lastClosedCandle && lastClosedCandle.time > p.entryTime) {
                    p.experimentalSL = lastClosedCandle.close;
                    p.experimentalSLSet = true;
                }
            }

            // 2. Elite Breakeven (Entry +/- Tick) - Active if we have moved past entry candle
            let eliteBESL = 0;
            const candlesSinceEntry = this.candles.filter(c => c.time >= p.entryTime).length;

            if (candlesSinceEntry > 0) { // At least one candle closed since entry
                if (p.type === 'LONG') {
            ...
            // 3. Determine SL (Hybrid)
            let finalSL = 0;
            let slType = 'None';

            const expSL = p.experimentalSLSet ? p.experimentalSL : (p.type === 'LONG' ? 0 : Infinity);
            const checkBE = eliteBESL !== 0 ? eliteBESL : (p.type === 'LONG' ? 0 : Infinity);

            if (p.type === 'LONG') {
                if (checkBE > expSL) {
                    finalSL = checkBE;
                    slType = 'Breakeven+Tick';
                } else {
                    finalSL = expSL;
                    slType = 'Experimental';
                }
            ...
            // 4. Delayed Trailing
            if (candlesSinceEntry >= trailingDelay) {
                const trailPct = parseFloat(this.trailingPct) || 1.0;
                const trailDist = trailPct / 100;

                if (p.type === 'LONG') {
                    const trailLevel = p.highestPrice * (1 - trailDist);
                    if (trailLevel > finalSL) {
                        finalSL = trailLevel;
                        slType = 'Delayed Trailing';
                    }
                } else {
                    const trailLevel = p.lowestPrice * (1 + trailDist);
                    if (trailLevel < finalSL || finalSL === 0) {
                        finalSL = trailLevel;
                        slType = 'Delayed Trailing';
                    }
                }
    ...
    checkPendingLimitOrder(tick) {
        ...
        if (filled) {
            console.log(`[${this.symbol}] BACKGUARD - Limit Order FILLED at ${price}`);
            this.openPosition(type, price, tick.time);
    ...
    handleSpiritReEntry(reason, lastType, exitPrice) {
        ...
        const isExperimental = this.strategy === 'SPIRIT_EXPERIMENTAL' || this.strategy === 'SPIRIT_EXPERIMENTAL_FIXED';
