...
class TradingEngine {
    ...
    manageOpenPosition(tick) {
        ...
        if (this.strategy === 'SPIRIT_TEST' && this.slMode === 'TRAILING') {
            ...
            // Backtest: simLow = min(lowestPrice, nextCandle.low) → effectiveSL → check nextCandle.high
            // Paper equivalent: update extremes with tick → calc SL → check hit (all in same isFinal)
            // This prevents premature SL triggers on the first tick of a new candle
            // before the candle's full range (low/high) has been incorporated.
            if (tick.isFinal) {
                ...
                // Emit SL update for mirror mode (only when SL actually changes)
                ...
                // Check hit ONLY on candle close, AFTER entry candle
                // (matches backtest stopActiveFromIndex candle-by-candle check)
