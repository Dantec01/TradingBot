...
class TradingEngine {
    ...
    manageOpenPosition(tick) {
        ...
        // SPIRIT_ELITE: Activation-based SL
        if (this.strategy === 'SPIRIT_ELITE') {
            const eliteActivationPct = this.eliteActivationPct || 0.002; // 0.2% default
            const eliteTickOffset = this.eliteTickOffset || 0.0001; // 1 tick default

            if (!p.eliteSLActivated) {
                if (p.type === 'LONG') {
                    const profitPct = (p.highestPrice - p.entryPrice) / p.entryPrice;
                    if (profitPct >= eliteActivationPct) {
                        p.eliteSLActivated = true;
                        p.eliteSLPrice = p.entryPrice + eliteTickOffset; // LONG: SL above entry (profit lock)
                        this.log('INFO', `[SPIRIT_ELITE] SL Activated at ${p.eliteSLPrice} (entry: ${p.entryPrice})`);
                    }
                } else {
                    const profitPct = (p.entryPrice - p.lowestPrice) / p.entryPrice;
                    if (profitPct >= eliteActivationPct) {
                        p.eliteSLActivated = true;
                        p.eliteSLPrice = p.entryPrice - eliteTickOffset; // SHORT: SL below entry (profit lock)
                        this.log('INFO', `[SPIRIT_ELITE] SL Activated at ${p.eliteSLPrice} (entry: ${p.entryPrice})`);
                    }
                }
            }
            ...
            if (p.eliteSLActivated) {
                effectiveSL = p.eliteSLPrice;
            } else {
                effectiveSL = 0; // No SL until activated
            }
        }
        ...
        if (p.type === 'LONG') {
            ...
            if (this.strategy === 'SPIRIT_ELITE' && p.eliteSLActivated && effectiveSL > 0 && tick.low <= effectiveSL) {
                hit = true;
            }
            ...
            if (this.strategy === 'SPIRIT_ELITE' && p.eliteSLActivated && effectiveSL > 0 && tick.high >= effectiveSL) {
                hit = true;
            }
        }
