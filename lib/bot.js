...
class TradingEngine {
        ...
        if (!this.position) {
            ...
            if (isTrigger || isSpiritReEntry) {
                ...
                if (canEnter) {
                    ...
                    if (this.strategy === 'BACKGUARD') {
                    ...
                    } else if (this.strategy === 'SPIRIT_ELITE' || this.strategy === 'SPIRIT_TRAILING') {
                        // SPIRIT_ELITE & SPIRIT_TRAILING: Explicit Aggressive Limit Order
                        // docs/estrategia_limit.md: Limit at 0.5% offset
                        const offset = 0.005; // 0.5%
                        const limitPrice = (type === 'LONG')
                            ? candle.close * (1 + offset)  // LONG: Comprar más caro (asegura fill inmediato)
                            : candle.close * (1 - offset); // SHORT: Vender más barato (asegura fill inmediato)
                        ...
                        console.log(`[${this.symbol}] ${this.strategy} - Aggressive Limit Order Placed at ${limitPrice.toFixed(4)} (${type})`);
                    } else {
    ...
    checkPendingLimitOrder(tick) {
        ...
        if (filled) {
            ...
            // For SPIRIT_ELITE and SPIRIT_TRAILING (Aggressive Limit), we fill at Market (tick.close) because Limit is "through" the price.
            // For others (Passive Limit), we fill at Limit Price (Conservative Paper Trading).
            let fillPrice = price;
            if (stratName === 'SPIRIT_ELITE' || stratName === 'SPIRIT_ELITE Re-entry' || stratName === 'SPIRIT_TRAILING') {
    ...
    handleSpiritReEntry(reason, lastType, exitPrice) {
        ...
        if (canReEnter) {
            ...
            if (this.strategy === 'SPIRIT_ELITE' || this.strategy === 'SPIRIT_TRAILING') {
                ...
                console.log(`[${this.symbol}] ${this.strategy} - Re-entry Wait (Aggressive Limit will be used). Waiting for ${neededSignal}.`);
            } else {
