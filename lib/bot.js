...
class TradingEngine {
    constructor(config) {
        ...
        // eliteTickOffset: Price offset added to entry for Breakeven+Tick SL
        //   - Default: 0.0001 (1 tick)
        //   - This ensures a tiny profit when BE is hit
        //   - For LONG: SL = entry + offset, for SHORT: SL = entry - offset
    ...
    manageOpenPosition(tick) {
        ...
        if (this.strategy === 'SPIRIT_ELITE') {
            ...
            if (p.candlesSinceEntry >= 1) { // 1 candle closed means we are in the "next" candle
                // Convert percentage offset to price value
                const offsetVal = p.entryPrice * (this.eliteTickOffset / 100);
