...
class TradingEngine {
    constructor(config) {
        ...
        // Elite Params
        this.eliteTrailingDefer = config.eliteTrailingDefer || 5;
            ...
            if (isTrigger || isSpiritReEntry) {
                ...
                if (canEnter) {
                    ...
                    if (this.strategy === 'BACKGUARD') {
                        ...
                        // SPIRIT_ELITE: Explicit Aggressive Limit Order
                        // docs/estrategia_limit.md: Limit at 0.5% offset
                        const offset = 0.005; // 0.5%
                        const limitPrice = (type === 'LONG')
                            ? candle.close * (1 + offset)
                            : candle.close * (1 - offset);
                        ...
                        console.log(`[${this.symbol}] SPIRIT_ELITE - Aggressive Limit Order Placed at ${limitPrice.toFixed(4)} (${type})`);
                    } else {
    ...
    manageOpenPosition(tick) {
        ...
        // BACKGUARD Logic: Update candle counter and manage SLs
        if (this.strategy === 'BACKGUARD') {
            if (tick.time > p.lastCandleTime) {
                p.candlesSinceEntry++;
                p.lastCandleTime = tick.time;
                // console.log(`[${this.symbol}] BACKGUARD Candle +1. Total: ${p.candlesSinceEntry}`);
            }
        ...
        if (this.strategy === 'SPIRIT_ELITE') {
            const eliteTickOffset = this.eliteTickOffset || 0.0001;

            // 1. Experimental SL (Dynamic)
            // Sets SL based on the candle that JUST closed after entry
            if (!p.experimentalSLSet && Date.now() > (p.entryTime + 1000)) { // Simple check: valid after entry
                // In Paper, we rely on `lastCandleTime` to know if a candle closed.
                if (p.lastCandleTime && p.lastCandleTime > p.entryTime) {
                    // Find that closed candle
                    const closedCandle = this.candles.find(c => c.time === p.lastCandleTime);
                    if (closedCandle) {
                        p.experimentalSL = closedCandle.close; // Or use strict experimental logic if needed
                        p.experimentalSLSet = true;
        ...
        if (p.type === 'LONG') {
            ...
            if (this.strategy === 'SPIRIT_ELITE' && effectiveSL > 0 && tick.low <= effectiveSL) {
                hit = true;
                // Reason will be handled below using p.eliteSLType
            }
    ...
    checkPendingLimitOrder(tick) {
        ...
        if (filled) {
            ...
            // For SPIRIT_ELITE (Aggressive Limit), we fill at Market (tick.close) because Limit is "through" the price.
            // For others (Passive Limit), we fill at Limit Price (Conservative Paper Trading).
            let fillPrice = price;
            if (stratName === 'SPIRIT_ELITE' || stratName === 'SPIRIT_ELITE Re-entry') {
    ...
    handleSpiritReEntry(reason, lastType, exitPrice) {
        ...
        if (canReEnter) {
            ...
            // SPIRIT_ELITE: Immediate Re-entry Logic (Explicit Aggressive Limit)
            if (this.strategy === 'SPIRIT_ELITE') {
                const offset = 0.005; // 0.5%
                // Use Exit Price as reference? Or better to wait for signal?
                // User requirement: "reentrar al cierre de la vela donde aparecio la nueva señal en limit"
                // This function runs on closePosition, which might be BEFORE the new signal is processed or part of it?
                // If this is a stop loss, we usually wait for next signal.
                // If this is a REVERSAL, processSignal handles entry.
                // Assuming this is for Re-Entry after a Stop/Trailing.

                // We will set pendingReEntry to enforce the signal check, BUT we tag it
                // so processSignal knows to use Aggressive Limit.
                // (Already handled in processSignal by checking strategy === 'SPIRIT_ELITE')

                this.pendingReEntry = {
                    neededSignal,
                    entryPrice: exitPrice
                };
                console.log(`[${this.symbol}] SPIRIT_ELITE - Re-entry Wait (Aggressive Limit will be used). Waiting for ${neededSignal}.`);
            } else {
                // Standard SPIRIT: Wait for signal confirmation
                this.pendingReEntry = {
                    neededSignal,
                    entryPrice: exitPrice // Use exit price as target if possible (or market)
                };
                console.log(`[${this.symbol}] SPIRIT - Re-entry Pending. Waiting for ${neededSignal} signal.`);
            }
            this.onStateChange();
        }
    }
