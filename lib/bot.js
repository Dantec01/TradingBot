...
class TradingEngine {
    constructor(config) {
        ...
        this.logger = config.onLog || ((l, m) => { });

        // EventEmitter for SPIRIT_TEST mirror mode (paper → real communication)
        this.events = new EventEmitter();

        // SPIRIT_TEST: Instant entry option (enter based on current signal, not waiting for change)
    ...
    processSignal() {
        ...
        if (!this.position) {
            ...
            // SPIRIT_TEST Instant Entry: treat current signal as trigger on first candle
            ...
            if (isTrigger || isSpiritReEntry || isInstantEntry) {
                // Auto-disable instant entry after first use
                if (isInstantEntry) {
                    this.instantEntry = false;
                    this.logger('INFO', `⚡ Entrada Inmediata: usando señal actual (${signal}) para entrar`);
                }
    ...
    manageOpenPosition(tick) {
        ...
        // ============================================================
        // SPIRIT_TEST: Match backtest behavior exactly
        // Backtest flow: update extremes on candle close → calc SL → check next candle
        // - Extremes & SL recalculated ONLY on candle close (isFinal)
        // - SL checked AFTER entry candle (skip entry candle)
        // ============================================================
        if (this.strategy === 'SPIRIT_TEST' && this.slMode === 'TRAILING') {
            // SPIRIT_TEST: Evaluate trailing SL ONLY on candle close (matches backtest exactly)
            // Skip entry candle entirely: position opened at candle.close, so the
            // intra-candle low/high before the close are irrelevant (position didn't exist yet).
            // Extremes only track from the first FULL candle after entry.
            if (tick.isFinal && tick.time > p.entryCandleTime) {
                ...
                const prevSL = p.trailingSLLevel || 0;

                p.trailingSLLevel = p.type === 'LONG'
                    ? p.highestPrice * (1 - trailingRate)
                    : p.lowestPrice * (1 + trailingRate);

                // Check hit FIRST — if hit, closePosition emits paper:close directly.
                // Do NOT emit paper:sl_update before paper:close (causes race condition in real bot).
                if (p.trailingSLLevel > 0) {
                    let hit = false;
                    if (p.type === 'LONG' && tick.low <= p.trailingSLLevel) hit = true;
                    else if (p.type === 'SHORT' && tick.high >= p.trailingSLLevel) hit = true;

                    if (hit) {
                        this.closePosition(p.trailingSLLevel, 'Trailing Stop');
                        return; // paper:close handles everything, no sl_update needed
                    }
                }

                // Only emit sl_update if position CONTINUES (no hit)
                if (p.trailingSLLevel !== prevSL) {
                    this.events.emit('paper:sl_update', {
                        symbol: this.symbol,
                        trailingSLLevel: p.trailingSLLevel,
                        type: p.type
                    });
                }
            }
            return; // SPIRIT_TEST trailing fully handled
        }
        ...
        if (this.slMode === 'TRAILING' && !hasCustomSL) {
            // On candle close: update extremes and recalculate trailing SL
            if (tick.isFinal) {
                if (p.type === 'LONG') p.highestPrice = Math.max(p.highestPrice || p.entryPrice, tick.high);
                else p.lowestPrice = Math.min(p.lowestPrice || p.entryPrice, tick.low);
    ...
    openPosition(type, price, candleTime) {
        ...
        // Emit event for SPIRIT_TEST mirror mode
        this.events.emit('paper:open', {
            symbol: this.symbol,
            type,
            price,
            candleTime,
        ...
        this.logger('INFO', `📤 OPEN ${type} @ ${price.toFixed(5)} → Mirror event emitted`);
    ...
    closePosition(price, reason) {
        ...
        // Emit event for SPIRIT_TEST mirror mode
        this.events.emit('paper:close', {
            symbol: this.symbol,
            price,
            reason
        });
        this.logger('INFO', `📤 CLOSE @ ${price.toFixed(5)} (${reason}) → Mirror event emitted`);
