const { fetchKlines, fetchFundingRate, fetchCommissionRate } = require('./binance');
...
class TradingEngine {
    ...
    async init() {
        ...
        const initialCandles = await fetchKlines(this.symbol + 'USDT', this.timeframe, warmupCount);
        this.candles = initialCandles;
        this.recalculateIndicators();

        // Fetch current funding rate
        this.currentFundingRate = await fetchFundingRate(this.symbol);

        // Fetch commission rates (dynamic from Binance if possible)
        await this.refreshRates();
        this.lastFundingUpdate = Date.now();

        console.log(`[Bot Engine] ${this.symbol} ready. Funding: ${this.currentFundingRate} | Taker: ${this.takerFee}`);
    }
    ...
    async refreshRates() {
        try {
            ...
            // console.log(`[${this.symbol}] Rates refreshed. Funding: ${this.currentFundingRate}`);
        } catch (err) {
    ...
    countFundingWindows(startTime, endTime) {
        ...
        // Find next 8h boundary
        let nextHour = Math.floor(current.getUTCHours() / 8) * 8 + 8;
    ...
    closePosition(price, reason) {
        ...
        // Realistic Funding Calculation (00:00, 08:00, 16:00 UTC)
        const windowsCrossed = this.countFundingWindows(p.entryTime, Date.now());
        let fundingFee = 0;
        if (windowsCrossed > 0) {
            const notional = p.size * price;
            fundingFee = notional * (this.currentFundingRate || 0) * windowsCrossed;
            if (p.type === 'SHORT') fundingFee = -fundingFee; // Shorts receive if rate > 0
        }

        const netPnL = pnlRaw - exitFee - p.entryCommission - fundingFee;

        // Since entryCommission was already subtracted in openPosition, 
        // we add back the margin plus the PnL (which already includes both fees for reporting)
        // Adjusting balance update to not double-subtract entryCommission
        this.balance += (p.margin + pnlRaw - exitFee - fundingFee);
