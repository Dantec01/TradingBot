...
class TradingEngine {
    ...
    update(newCandle) {
        ...
        // Execute pending reversal entry at the start of the new candle (matches backtest)
        if (this.pendingReversalEntry && newCandle.time > this.pendingReversalEntry.signalTime) {
            const { type } = this.pendingReversalEntry;
            this.openPosition(type, newCandle.open, newCandle.time);
            this.pendingReversalEntry = null;
        }
    ...
    processSignal() {
        ...
        if (this.position) {
            ...
            if (isTrigger && signal === oppositeSignal) {
                ...
                // Defer new entry to the start of the next candle (matches backtest: nextCandle.open)
                const newType = (signal === 'Buy') ? 'LONG' : 'SHORT';
                const canEnter = (newType === 'LONG' && (this.direction === 'BOTH' || this.direction === 'LONG')) ||
                    (newType === 'SHORT' && (this.direction === 'BOTH' || this.direction === 'SHORT'));
                if (canEnter) {
                    this.pendingReversalEntry = { type: newType, signalTime: candle.time };
                }
                return;
            }
        }
