...
class TradingEngine {
            ...
            if (isTrigger || isSpiritReEntry) {
                ...
                if (canEnter) {
                    ...
                    if (this.strategy === 'BACKGUARD' || this.strategy === 'SPIRIT_ELITE') {
                        // Place Virtual Limit Order at Close Price
                        this.pendingLimitOrder = {
                            type,
                            price: candle.close,
                            time: candle.time,
                            strategy: this.strategy // Tag with strategy
                        };
    ...
    checkPendingLimitOrder(tick) {
        ...
        if (filled) {
            ...
            console.log(`[${this.symbol}] ${stratName} - Limit Order FILLED at ${price}`);
    ...
    handleSpiritReEntry(reason, lastType, exitPrice) {
        ...
        if (canReEnter) {
            ...
            if (this.strategy === 'SPIRIT_ELITE') {
                // Direct re-entry setup for Limit Order
                this.pendingLimitOrder = {
                    type: lastType,
                    price: exitPrice,
                    time: Date.now(),
                    strategy: 'SPIRIT_ELITE Re-entry'
                };
                console.log(`[${this.symbol}] SPIRIT_ELITE - Re-entry Limit Order Placed at ${exitPrice}`);
            } else {
