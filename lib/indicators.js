...
function calculateRangeFilter(candles, config = {}) {
    ...
    if (!candles || candles.length === 0) {
        console.warn('[Indicators] No candles provided');
        return [];
    }
    ...
    if (n < period) {
        console.warn(`[Indicators] Insufficient data: ${n} candles, need ${period}`);
        return candles.map(() => ({ rngfilt: 0, signalStr: 'Neutral', isTrigger: false, state: 0 }));
    }
