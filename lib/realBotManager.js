...
class BotManager {
    ...
    connect() {
        ...
        this.ws.on('message', (data) => {
            try {
                ...
                // Non-JSON messages (ping/pong, binary) - log for visibility
        ...
        this.ws.on('close', () => {
            ...
            console.log(`[RealBot Manager] WS Closed. Reconnecting in ${backoff / 1000}s (Attempt ${this.reconnectAttempts})`);
    ...
    async connectUserDataStream() {
        ...
        try {
            ...
            const listenKeyResponse = await this.binanceClient.futuresGetListenKey();
            this.listenKey = listenKeyResponse;
            console.log(`[RealBot Manager] User Data Stream listenKey obtained`);
            this.addLog('INFO', 'User Data Stream listenKey obtained');
            ...
            this.userDataWs.on('open', () => {
                ...
                this.addLog('SUCCESS', 'User Data Stream connected (real-time fills enabled)');
                this.userDataReconnectAttempts = 0;
                ...
                this.keepAliveInterval = setInterval(async () => {
                    try {
                        await this.binanceClient.futuresKeepListenKey();
            ...
            this.userDataWs.on('close', () => {
                console.log('[RealBot Manager] User Data Stream closed');
                this.addLog('WARN', 'User Data Stream disconnected');
            ...
            this.userDataWs.on('error', (err) => {
                ...
                this.addLog('ERROR', `User Data Stream error: ${err.message}`);
            });

        } catch (err) {
            console.error('[RealBot Manager] Failed to connect User Data Stream:', err.message);
            this.addLog('ERROR', `User Data Stream connection failed: ${err.message}`);
    ...
    handleOrderTradeUpdate(order) {
        ...
        this.addLog('INFO', `Order Update: ${symbol} ${status} @ ${avgPrice} (ID: ${orderId})`);

        // Check if this is a fill for a pending limit order
        if (engine.waitingForLimitFill && engine.waitingForLimitFill.orderId === orderId) {
            if (status === 'FILLED') {
                ...
                this.addLog('SUCCESS', `LIMIT ORDER FILLED: ${symbol} @ ${avgPrice}`);
            } else if (status === 'CANCELED' || status === 'EXPIRED' || status === 'REJECTED') {
                // Order was not filled
                engine.onOrderCanceled(status);
                this.addLog('WARN', `Order ${orderId} ended: ${status}`);
            }
            // Log other order statuses for visibility
            else if (status === 'NEW') {
                this.addLog('INFO', `Order ${orderId} created and active`);
            }
            else if (status === 'PARTIALLY_FILLED') {
                this.addLog('INFO', `Order ${orderId} partially filled: ${executedQty}`);
            }
        }
    }
}
