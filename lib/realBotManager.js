...
class BotManager {
            ...
            const dataToSave = {
                bots: status.bots.map(b => ({
                    ...
                    state: {
                        ...
                        activeStopOrder: b.activeStopOrder, // STOP_MARKET order on Binance
    ...
    handleOrderTradeUpdate(order) {
        ...
        // Check if this is a fill for a STOP_MARKET order (SL execution)
        if (engine.activeStopOrder && engine.activeStopOrder.orderId === orderId) {
            if (status === 'FILLED') {
                this.addLog('SUCCESS', `✓ STOP_MARKET ejecutado: ${symbol} @ ${avgPrice}`);
                ...
                this.addLog('INFO', `STOP_MARKET ${symbol} cancelado/expirado`);
