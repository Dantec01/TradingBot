...
class BotManager {
    ...
    handleOrderTradeUpdate(order) {
        ...
        // Check if this is a fill for a STOP_MARKET or TAKE_PROFIT_MARKET order (SL/TP execution)
        if (engine.activeStopOrder && engine.activeStopOrder.orderId === orderId) {
            const typeLabel = engine.activeStopOrder.orderType === 'TAKE_PROFIT_MARKET' ? 'TP_MARKET' : 'STOP_MARKET';
            if (status === 'FILLED') {
