...
class BotManager {
    constructor() {
        ...
        this.heartbeatTimeout = 120000; // 120 seconds - if no message, consider dead (tolerant for network hiccups)
    ...
    connect() {
        ...
        this.ws.on('open', () => {
            ...
            // Setup heartbeat: active ping + dead connection detection
            // Send ping every 30s to keep connection alive, check for timeout every 30s
            this.heartbeatInterval = setInterval(() => {
                ...
                if (timeSinceLastMessage > this.heartbeatTimeout) {
                    ...
                    // Send active ping to keep connection alive
        ...
        this.ws.on('close', (code, reason) => {
            ...
            const baseBackoff = Math.min(Math.pow(2, this.reconnectAttempts) * 1000, this.maxBackoff);
            const jitter = Math.random() * 1000;
            const backoff = baseBackoff + jitter;

            console.log(`[RealBot Manager] WS Closed (code: ${code}, reason: ${reason}). Reconnecting in ${(backoff / 1000).toFixed(1)}s (Attempt ${this.reconnectAttempts})`);
            this.addLog('WARN', `WebSocket cerrado. Reconectando en ${Math.ceil(backoff / 1000)}s...`);
