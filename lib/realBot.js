...
class TradingEngine {
    ...
    async manageOpenPosition(tick) {
        ...
        if (this.strategy === 'SPIRIT_TEST' && this.mirrorMode) {
            // Place TRAILING_STOP_MARKET only after activation conditions are met
            if (tick.isFinal && tick.time > p.entryCandleTime && !this._trailingStopPlaced) {
                // If trailingActivation is set, wait for price to move in our favor
                if (this.trailingActivation > 0) {
                    ...
                    if (!priceReached) {
                        // Not yet at activation target — skip placement this candle
                        return;
                    }
                    this.log('INFO', `✅ Activación alcanzada: precio llegó a ${targetPrice.toFixed(6)} → colocando trailing`);
                }
