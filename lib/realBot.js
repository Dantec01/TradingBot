...
class TradingEngine {
    ...
    async update(newCandle) {
        ...
        // SPIRIT_TEST: Opens MARKET using own indicators
        // Real bot loads 500 candles → indicators match paper bot
        // Exit handled by own tick-by-tick trailing stop
        // ============================================================
        if (this.independentMode && !this.isClosing && !this._isEntering && newCandle.eventTime && newCandle.closeTime) {
            ...
            if (timeToClose > 0 && timeToClose <= 1500) {
                // --- PREEMPTIVE ENTRY ---
                // --- PREEMPTIVE ACTION (Entry or Reversal) ---
                if (!this.waitingForLimitFill && !this._preemptiveEntryDone) {
                    ...
                    if (isTrigger || isSpiritReEntry || isInstantEntry || manualTrigger) {
                        ...
                        if (this.position && this.position.type !== type && !this.isClosing) {
                        ...
                        // Only if no position (either was null or just closed above)
                        if (canEnter && !this.position && timeToClose <= 1200 && !this._preemptiveEntryDone) {
    ...
    async processSignal() {
        ...
        if (this.independentMode) {
            // Only process reversals — entry is handled by preemptive T-1.2s block
            const candle = this.candles[this.candles.length - 1];
            const signal = candle.signalStr;
            const isTrigger = candle.isTrigger;
            ...
            if (this.position && isTrigger) {
                ...
                if (signal === oppositeSignal) {
                    ...
                    // Do NOT re-enter here — preemptive entry handles new entries
                }
            }
            return;
        }
