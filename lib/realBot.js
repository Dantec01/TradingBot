...
class TradingEngine {
    ...
    async processAuxiliaryCandle(candle1m) {
        ...
        // Condition 3: Single 1m Reversal + Wick Exit
        // 1 opposite signal (current state matches needed reversal)
        const isOpposite = (neededReversal === 'Sell' && lastSig.state === -1) ||
            (neededReversal === 'Buy' && lastSig.state === 1);
        ...
        if (isOpposite) {
            ...
            if (positionType === 'LONG') {
                // Expecting bearish reversal. Check Upper Wick above RF?
                // Logic: "Vela q sobresalga un poco del rango".
                // If RF is the "band", and price is ABOVE it for a LONG... wait.
                // Range Filter: In "Sell" state, RF is ABOVE price (usually acting as resistance).
                // So if High > RF, we pierced resistance?
                // Actually Range Filter:
                // When Long (Buy): Price > RF. 
                // When Short (Sell): Price < RF.
                // So if we are Long, we want to exit on Short signal.
                // Short signal means Price < RF.
                // The "Wick" condition implies we tried to go back UP and failed? or tried to go DOWN?
                // User said: "Vela q sobresalga un poco del rango... (lo que nose es cuanto pero he notado que estas velas dejan colas largar por lo que en algun momento esa vela salio del rango)"
                // "Cerrar inmediatamente mientras mas sobresalga"
                // If we are LONG, looking for SELL.
                // A Sell signal in RF means Close < RF.
                // If "sobresale del rango", maybe means High > RF? (It closed below, but wick went above).
                // This seems like a rejection of the high.

                // Correction: User might mean "Outside the range" during the reversal.
                // If we are transitioning Buy -> Sell.
                // Range Filter line flips or price crosses it.

                // Interpretation:
                // LONG Exit -> We want a SELL signal (Price < RF).
                // "Wick outside range": Maybe the Low punctured deep? Or High tried to stay up?
                // Let's stick to the interpretation: "Wick against the new trend is large" OR "Wick penetrating the filter".
                // Let's use: If candle penetrates the filter significantly.
                // Since it's a SELL signal, Close is < RF.
                // "Sobresale un poco": High > RF (it was above, then closed below).
                // This seems like a rejection of the high.

                if (candle1m.high > rfValue) {
                    wickBreach = true;
                    this.log('INFO', `[VANGUARD] Wick Breach Detected (LONG Exit): High ${candle1m.high} > RF ${rfValue}`);
                }

            } else {
                // SHORT Exit -> Want BUY signal (Price > RF).
                // Close > RF.
                // "Sobresale un poco": Low < RF (it was below, then closed above).
                if (candle1m.low < rfValue) {
                    wickBreach = true;
                    this.log('INFO', `[VANGUARD] Wick Breach Detected (SHORT Exit): Low ${candle1m.low} < RF ${rfValue}`);
                }
            }

            if (wickBreach) {
                this.log('INFO', `[VANGUARD] Exit Condition 3: 1m Wick Exit triggered.`);
                await this.closePosition(candle1m.close, 'Vanguard Wick Exit');
                return;
            }
        }
    }
    ...
    handleSpiritReEntry(reason, lastType, exitPrice) { // SPIRIT RE-ENTRY Setup
        const isStopLoss = reason.startsWith('Stop Loss') || reason.startsWith('Breakeven SL') || reason.startsWith('Stop Loss (Shield)');
        const isTrailing = reason.startsWith('Trailing Stop');
        const isSpirit = this.strategy.startsWith('SPIRIT');

        if (!(isSpirit && (isStopLoss || (isTrailing && this.strategy === 'SPIRIT_TRAILING')))) {
            return;
        }
