    ...
    linkToPaperBot(paperEngine) {
        ...
        paperEngine.events.on('paper:close', async (data) => {
            ...
            if (this.waitingForLimitFill) {
                // Before cancelling, check if order actually filled (race condition safety)
                try {
                    if (this.waitingForLimitFill.orderId) {
                        ...
                        if (orderStatus.status === 'FILLED') {
                            this.log('INFO', `🪞 Mirror: Order was actually FILLED! Processing fill before close...`);
                            ...
                            // Now position exists, fall through to close below
                        }
                    }
                } catch (checkErr) {
                    this.log('WARN', `Could not verify order status: ${checkErr.message}`);
                }

                // If still waiting (order wasn't filled), cancel it
                if (this.waitingForLimitFill) {
