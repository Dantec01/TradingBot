...
class TradingEngine {
    ...
    manageOpenPosition(tick) {
        ...
        if (this.strategy === 'SPIRIT_ELITE') {
            ...
            // 3. Delayed Trailing (after X candles)
            let trailingSL = 0;
            const deferCandles = this.eliteTrailingDefer || 5; // Default to 5 if not set
            if (candlesStored >= deferCandles) {
    ...
    handleSpiritReEntry(reason, lastType, exitPrice) { // SPIRIT RE-ENTRY Setup
        ...
        if (canReEnter) {
            ...
            // SPIRIT_ELITE update: Now behaves like Experimental (Wait for Signal)
            // But uses Aggressive Limit Entry (handled in openPosition)
            // So we just queue the re-entry like standard Spirit.
            ...
            this.log('INFO', `${this.strategy} - Re-entry queued. Waiting for ${neededSignal} signal (Aggressive Limit Entry).`);
