...
class TradingEngine {
    ...
    async update(newCandle) {
        ...
        if (this.independentMode && !this.isClosing && !this._isEntering && newCandle.eventTime && newCandle.closeTime) {
            ...
            // If we are far away, set a timeout to awake us exactly at T-1500ms / T-1200ms
            if (timeToClose > 1600 && !this._preemptiveTimerSet) {
                const msUntilWindow = timeToClose - 1500;
                ...
                if (msUntilWindow > 0 && msUntilWindow < 300000) {
                    ...
                    // console.log(`[RealBot Timer] Armado para dentro de ${msUntilWindow}ms (T-1.5s real)`);
                    ...
                    this._preemptiveTimeoutId = setTimeout(async () => {
                        ...
                        // Fake a tick with time = closeTime - 1400 (places us right in the window)
                        const lastCandle = this.candles[this.candles.length - 1];
                        const signalStr = lastCandle.signalStr;
                        // For debug log
                        this.log('INFO', `⏱️ [TIMER] Forzando ventana anticipada! señal actual=${signalStr}`);

                        // Ejecutamos la lógica de preemptive directamente
                        await this.executePreemptiveLogic(1400);
                    }, msUntilWindow);
                }
            }

            if (timeToClose > 0 && timeToClose <= 1500) {
    ...
    async manageOpenPosition(tick) {
        ...
        if (this.strategy === 'SPIRIT_TEST' && this.independentMode) {
            ...
            if (activationReached) {
                ...
                // Binance backup is NOT used — our trailing handles all exits
            }
