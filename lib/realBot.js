...
class TradingEngine {
            ...
            if (isTrigger || isSpiritReEntry) {
                ...
                if (canEnter) {
                    ...
                    if (this.strategy === 'BACKGUARD') {
                        ...
                        // SPIRIT_ELITE: Virtual Order + MARKET Execution
                        // Simulates paper bot logic locally, executes MARKET when conditions are met
                        // This ensures IDENTICAL behavior to paper bot
                        const offset = 0.005; // 0.5% offset (same as paper)
                        const limitPrice = (type === 'LONG')
                            ? targetPrice * (1 + offset)
                            : targetPrice * (1 - offset);
                        
                        this.pendingVirtualOrder = {
                            type,
                            price: limitPrice,
                        ...
                        this.log('INFO', `🎯 SPIRIT_ELITE - Orden VIRTUAL: ${type} | Base: ${targetPrice.toFixed(decimals)} | Limit: ${limitPrice.toFixed(decimals)} (offset ${(offset * 100).toFixed(1)}%)`);
                    } else {
    ...
    async manageOpenPosition(tick) {
        ...
        // Update STOP_MARKET order on Binance when SL changes
        // This ensures precise execution at the SL price instead of market price
        if (effectiveSL > 0 && !hit) {
            await this.updateStopMarket(effectiveSL);
        }

        if (hit) {
            // If we detect a hit locally but have an active stop order, 
            // the stop should execute on Binance. Cancel and close manually only as fallback.
     ...
     */
    async placeStopMarket(stopPrice) {
        if (!this.position) return;
        ...
        try {
            ...
            this.log('INFO', `📌 Colocando STOP_MARKET @ ${roundedStopPrice.toFixed(decimals)}...`);
            ...
            this.log('SUCCESS', `✓ STOP_MARKET colocado: ID ${order.orderId} @ ${roundedStopPrice.toFixed(decimals)}`);
    ...
    async updateStopMarket(newStopPrice) {
        ...
        this.log('INFO', `🔄 Actualizando STOP_MARKET: ${this.activeStopOrder?.stopPrice?.toFixed(decimals) || 'N/A'} → ${roundedNewPrice.toFixed(decimals)}`);
        
        // Cancel and replace
        await this.placeStopMarket(roundedNewPrice);
    }
    ...
    /**
     * Check if position was closed by STOP_MARKET (called on order update events)
     */
    async handleStopMarketFilled(orderData) {
        ...
        this.log('SUCCESS', `✓ STOP_MARKET ejecutado @ ${orderData.avgPrice}`);
        ...
        // Determine reason based on eliteSLType
    ...
    handleSpiritReEntry(reason, lastType, exitPrice) { // SPIRIT RE-ENTRY Setup
        ...
        if (canReEnter) {
            ...
            // SPIRIT_ELITE & SPIRIT_TRAILING: Immediate Re-entry Logic (Explicit Aggressive Limit)
            if (this.strategy === 'SPIRIT_ELITE' || this.strategy === 'SPIRIT_TRAILING') {
                // We will set pendingReEntry to enforce the signal check, BUT we tag it
                // so processSignal knows to use Aggressive Limit.
                // (Already handled in processSignal by checking strategy)

                this.pendingReEntry = {
                    neededSignal,
                    entryPrice: exitPrice
                };
                this.log('INFO', `${this.strategy} - Re-entry Wait (Aggressive Limit will be used). Waiting for ${neededSignal}.`);
            } else {
                ...
                this.log('INFO', `SPIRIT - Re-entry Pending. Waiting for ${neededSignal} signal.`);
            }
            this.onStateChange();
        }
    }
