...
class TradingEngine {
    ...
    linkToPaperBot(paperEngine) {
        ...
        paperEngine.events.on('paper:open', async (data) => {
            ...
            // If we already have a position, pending order, or active trailing → skip
            if (this.position || this.waitingForLimitFill || this.activeStopOrder) {
                this.log('INFO', `🪞 Paper abrió pero real ya tiene posición/trailing activo → skip`);
                return;
            }
    ...
    async update(newCandle) {
        ...
        // Reads PAPER BOT's indicators (reliable, same candle history)
        // Opens MARKET 1s before paper would detect the signal
        // Exit handled by Binance native TRAILING_STOP_MARKET
        // ============================================================
        if (this.preemptiveCloseEnabled && !this.isClosing && newCandle.eventTime && newCandle.closeTime) {
            const timeToClose = newCandle.closeTime - newCandle.eventTime;
            if (timeToClose > 0 && timeToClose <= 1000) {
                // --- PREEMPTIVE ENTRY ---
                if (!this.position && !this.waitingForLimitFill && !this._preemptiveEntryDone) {
                    // Use PAPER bot's indicators (they have correct candle history)
                    const paperCandles = this.paperBotRef ? this.paperBotRef.candles : this.candles;
                    const candle = paperCandles[paperCandles.length - 1];
                    const prevCandle = paperCandles.length >= 2 ? paperCandles[paperCandles.length - 2] : null;
                    ...
                    // DEBUG: Log all conditions at T-1s window
                    this.log('INFO', `🔍 PRE-ENTRY CHECK ${timeToClose}ms | signal=${signal} state=${candle.state} prevState=${prevCandle?.state} | isTrigger=${isTrigger} manualTrigger=${manualTrigger} reEntry=${!!isSpiritReEntry} instant=${!!isInstantEntry} | pos=${!!this.position} done=${this._preemptiveEntryDone} | src=${this.paperBotRef ? 'PAPER' : 'LOCAL'}`);
                    ...
                    if (isTrigger || isSpiritReEntry || isInstantEntry || manualTrigger) {
                        ...
                        if (canEnter) {
                            ...
                            this.log('INFO', `🔍 PRE-ENTRY: señal ${type} bloqueada por dirección=${this.direction}`);
                        }
                    }
                }
    ...
    async manageOpenPosition(tick) {
        ...
        if (this.strategy === 'SPIRIT_TEST' && this.mirrorMode) {
            // Place TRAILING_STOP_MARKET on first candle close after entry
            if (tick.isFinal && tick.time > p.entryCandleTime && !this._trailingStopPlaced) {
                await this.placeTrailingStopNative();
                this._trailingStopPlaced = true; // Prevent repeated placement
            }
            return;
        }
    ...
    async placeTrailingStopNative() {
        ...
        try {
            ...
            // Calculate activation price if configured
            let activationPrice = null;
            if (this.trailingActivation > 0 && this.position) {
                const activationRate = this.trailingActivation / 100;
                if (this.position.type === 'LONG') {
                    activationPrice = this.position.entryPrice * (1 + activationRate);
                } else {
                    activationPrice = this.position.entryPrice * (1 - activationRate);
                }
                // Round to tick size
                const tickSize = this.exchangeInfo?.tickSize || 0.0001;
                const pricePrecision = this.getPrecision(tickSize);
                activationPrice = parseFloat(activationPrice.toFixed(pricePrecision));
            }

            this.log('INFO', `\ud83d\udccc Colocando TRAILING_STOP_MARKET: callbackRate=${callbackRate}%${activationPrice ? `, activación=${activationPrice}` : ''}...`);
