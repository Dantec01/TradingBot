...
class TradingEngine {
            ...
            if (isTrigger || isSpiritReEntry) {
                ...
                if (canEnter) {
                    ...
                    if (this.strategy === 'BACKGUARD') {
                        ...
                        await this.placeLimitOrder(type, limitPrice, candle.time, 0, true); // isElite = true
                    } else {
    ...
    async placeLimitOrder(type, price, candleTime, initialSL = 0, isElite = false) { // Added isElite parameter
        ...
        try {
            const side = type === 'LONG' ? 'BUY' : 'SELL';
            const strategyLabel = isElite ? 'SPIRIT_ELITE Aggressive' : 'BACKGUARD';
    ...
    handleSpiritReEntry(reason, lastType, exitPrice) { // SPIRIT RE-ENTRY Setup
        ...
        if (canReEnter) {
            ...
            if (this.strategy === 'SPIRIT_ELITE') {
                ...
                this.log('INFO', `SPIRIT_ELITE - Re-entry Wait (Aggressive Limit will be used). Waiting for ${neededSignal}.`);
            } else {
