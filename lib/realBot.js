...
class TradingEngine {
    ...
    async update(newCandle) {
        ...
        // ============================================================
        // PREEMPTIVE CLOSE: Close position 2s before candle close
        // Uses Binance server time (eventTime) to avoid local clock skew
        // Only for SPIRIT_TEST + TRAILING + trailingPct <= 0.5%
        // ============================================================
        if (this.preemptiveCloseEnabled && !this.isClosing && newCandle.eventTime && newCandle.closeTime) {
            ...
            if (timeToClose > 0 && timeToClose <= 2000) {
                // Close open position
                if (this.position) {
                    this.log('INFO', `⚡ Cierre anticipado: ${timeToClose}ms antes del cierre de vela @ ${newCandle.close}`);
                    await this.closePosition(newCandle.close, 'Preemptive Close');
                }
                // Cancel unfilled LIMIT order
                if (this.waitingForLimitFill) {
                    this.log('INFO', `⚡ Cancelando LIMIT no llenado: ${timeToClose}ms antes del cierre de vela`);
                    await this.cancelPendingOrder('Preemptive Cancel (candle closing)');
                }
            }
        }
