...
class TradingEngine {
            ...
            if (isTrigger || isSpiritReEntry) {
                ...
                if (canEnter) {
                    ...
                    if (this.strategy === 'BACKGUARD') {
                        ...
                        // SPIRIT_ELITE: Aggressive Limit Order (+0.5% offset)
                        // This helps pay maker fees instead of taker fees
                        const offset = 0.005; // 0.5%
                        const limitPrice = (type === 'LONG')
                            ? targetPrice * (1 + offset)  // LONG: Comprar más caro (asegura fill inmediato)
                            : targetPrice * (1 - offset); // SHORT: Vender más barato (asegura fill inmediato)
                        this.log('INFO', `SPIRIT_ELITE - Placing Aggressive Limit ${type} at ${limitPrice.toFixed(4)} (base: ${targetPrice})`);
                        await this.placeLimitOrder(type, limitPrice, candle.time, 0, true);
                    } else {
    ...
    async manageOpenPosition(tick) {
        ...
        if (this.strategy === 'SPIRIT_ELITE') {
            ...
            if (p.candlesSinceEntry >= 1) { // 1 candle closed means we are in the "next" candle
                ...
                const offsetVal = p.entryPrice * (eliteTickOffset / 100);
    ...
    async onOrderFilled(orderData) {
        ...
        this.log('SUCCESS', `USER DATA STREAM: Order ${orderId} FILLED @ ${orderData.avgPrice}`);
