...
class TradingEngine {
    constructor(config) {
        ...
        // Preemptive Close: close position 2s before candle close (Binance time)
        // Only for SPIRIT_TEST + TRAILING + trailingPct <= 0.5%
        this.preemptiveCloseEnabled = this.mirrorMode && this.slMode === 'TRAILING' && this.trailingPct <= 0.5;
    ...
    async manageOpenPosition(tick) {
        ...
        if (this.strategy === 'SPIRIT_TEST' && this.mirrorMode) {
            ...
            if (tick.isFinal && tick.time > p.entryCandleTime && !this.activeStopOrder) {
    ...
    async placeTrailingStopNative() {
        ...
        try {
            ...
            if (!order.orderId) {
                this.log('WARN', `TRAILING_STOP_MARKET placed but no orderId — will not be cancellable`);
                this.activeStopOrder = null;
            } else {
                this.log('SUCCESS', `✓ TRAILING_STOP_MARKET colocado: ID ${order.orderId}, callbackRate=${callbackRate}%`);
            }
