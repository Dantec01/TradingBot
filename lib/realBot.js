...
class TradingEngine {
    ...
    async manageOpenPosition(tick) {
        ...
        if (this.strategy === 'SPIRIT_TEST' && this.mirrorMode) {
            // Place TRAILING_STOP_MARKET only after price moves in our favor
            // Minimum activation = callbackRate so trailing can NEVER close below entry
            if (tick.isFinal && tick.time > p.entryCandleTime && !this._trailingStopPlaced) {
                const callbackRate = Math.max(0.1, Math.min(this.trailingPct, 5));
                // Use the higher of trailingActivation or callbackRate as minimum
                const minActivation = Math.max(this.trailingActivation || 0, callbackRate);
                const activationRate = minActivation / 100;
                const targetPrice = p.type === 'LONG'
                    ? p.entryPrice * (1 + activationRate)
                    : p.entryPrice * (1 - activationRate);
                const priceReached = p.type === 'LONG'
                    ? tick.high >= targetPrice
                    : tick.low <= targetPrice;

                if (!priceReached) {
                    return; // Not yet at activation target — wait
                }
                this.log('INFO', `✅ Activación alcanzada (${minActivation}%): precio llegó a ${targetPrice.toFixed(6)} → colocando trailing`);
                await this.placeTrailingStopNative();
     ...
     * Used for SPIRIT_TEST to let Binance manage trailing tick-by-tick
     */
    async placeTrailingStopNative() {
        if (!this.position) return;
