...
class TradingEngine {
    constructor(config) {
        ...
        this.trailingActivation = (config.trailingActivation !== undefined && config.trailingActivation !== '') ? parseFloat(config.trailingActivation) : 0;
        this.useFixedSL = config.useFixedSL || false;
        this.useTrailing = config.useTrailing || false;
        this.direction = config.direction || 'BOTH';
        this.instantEntry = config.instantEntry || false;
        this.useNativeTrailing = config.useNativeTrailing || false;
        ...
        // SPIRIT_TEST Independent Mode: own indicators + own trailing stop
        // No paper bot dependency — entry by own signals, exit by tick-by-tick trailing
        this.independentMode = this.strategy === 'SPIRIT_TEST';
        this.paperBotRef = null;
        this._trailingStopPlaced = false;
        this.intervalMs = this._parseIntervalMs(this.timeframe);

        // Independent mode: manual instant entry only (removed forced auto-entry)
        // if (this.independentMode) this.instantEntry = true;
    }
    ...
    // =============================================
    // PAPER BOT LINKING (Non-SPIRIT_TEST strategies)
    // =============================================
    // For non-SPIRIT_TEST strategies: links real bot to paper bot for LIMIT mirroring.
    // SPIRIT_TEST uses its own indicators — no paper bot dependency.

    linkToPaperBot(paperEngine) {
        if (this.independentMode) {
            this.log('INFO', `🔧 SPIRIT_TEST independiente: sin mirror, usando indicadores propios`);
            return;
        }
        ...
        // When paper opens a position → mirror with LIMIT order
        paperEngine.events.on('paper:open', async (data) => {
            if (data.symbol !== this.symbol.replace('USDT', '')) return;

            if (this.position || this.waitingForLimitFill || this.activeStopOrder || this._isEntering) {
                this.log('INFO', `🪞 Paper abrió pero real ya tiene posición/orden activa → skip`);
                return;
            }
        ...
        paperEngine.events.on('paper:sl_update', async (data) => {
            ...
            if (this.position) {
                ...
                await this.updateStopMarket(data.trailingSLLevel, currentPrice);
            }
        });

        // When paper closes → mirror close
        paperEngine.events.on('paper:close', async (data) => {
            if (data.symbol !== this.symbol.replace('USDT', '')) return;

            // Non-independent mode: original mirror behavior
            if (this.waitingForLimitFill) {
                try {
                    if (this.waitingForLimitFill.orderId) {
                        const orderStatus = await this.client.futuresOrder({
                            symbol: this.symbol,
                            orderId: this.waitingForLimitFill.orderId
                        });
                        if (orderStatus.status === 'FILLED') {
                            this.log('INFO', `🪞 Mirror: Orden FILLED! Procesando antes de cerrar...`);
                            await this.onOrderFilled({
                                orderId: parseInt(orderStatus.orderId),
                                avgPrice: parseFloat(orderStatus.avgPrice),
                    ...
                    this.log('WARN', `No se pudo verificar estado de orden: ${checkErr.message}`);
                }

                if (this.waitingForLimitFill) {
                    this.log('INFO', `🪞 Mirror: Paper cerró (${data.reason}) → Cancelando LIMIT`);
                    await this.cancelPendingOrder('Paper closed');
                }
            }

            if (this.position) {
                this.log('INFO', `🪞 Mirror: Paper cerró (${data.reason}) → Cerrando real`);
    ...
    async update(newCandle) {
        ...
        // ============================================================
        // PREEMPTIVE ENTRY: 1.2s before candle close (Binance time)
        // GUARANTEED TIMERS: Websocket ticks are NOT guaranteed to fire exactly at T-1200ms
        // Thus we set an internal setTimeout if we receive a tick earlier.
        // ============================================================
        if (this.independentMode && !this.isClosing && !this._isEntering && newCandle.eventTime && newCandle.closeTime) {
            const timeToClose = newCandle.closeTime - newCandle.eventTime;

            // --- GUARANTEE TIMERS (Predictive execution) ---
            // If we are far away, set a timeout to awake us exactly at T-1200ms
            if (timeToClose > 1300 && !this._preemptiveTimerSet) {
                const msUntilWindow = timeToClose - 1200;
                // Only set timer if it's within realistic bounds (e.g., less than 5 mins away)
                if (msUntilWindow > 0 && msUntilWindow < 300000) {
                    this._preemptiveTimerSet = true;

                    // Clear previous timer just in case
                    if (this._preemptiveTimeoutId) clearTimeout(this._preemptiveTimeoutId);

                    this._preemptiveTimeoutId = setTimeout(async () => {
                        this._preemptiveTimerSet = false;
                        if (!this.candles || this.candles.length === 0) return;

                        const lastCandle = this.candles[this.candles.length - 1];
                        const signalStr = lastCandle.signalStr;
                        // For debug log
                        this.log('INFO', `⏱️ [TIMER] Forzando ventana anticipada (T-1.2s)! señal actual=${signalStr}`);

                        // Ejecutamos la lógica de preemptive directamente pasando un tiempo válido para ENTRY (<=1300)
                        await this.executePreemptiveLogic(1100);
                    }, msUntilWindow);
                }
            }

            if (timeToClose > 0 && timeToClose <= 1300) {
                // Si llegamos aquí gracias a un tick REAl que cayó exactamente en la ventana, 
                // cancelamos cualquier timer pendiente
                if (this._preemptiveTimeoutId) {
                    clearTimeout(this._preemptiveTimeoutId);
                    this._preemptiveTimeoutId = null;
                    this._preemptiveTimerSet = false;
                }

                await this.executePreemptiveLogic(timeToClose);
            }
        }

        // Reset preemptive entry flag on new candle
        if (newCandle.isFinal) {
            this._preemptiveEntryDone = false;
            if (this._preemptiveTimeoutId) {
                clearTimeout(this._preemptiveTimeoutId);
                this._preemptiveTimeoutId = null;
            }
        ...
        // No preemptive close — Binance TRAILING_STOP_MARKET handles exit
    ...
    /**
     * Executes the preemptive logic (reversals at <=1500ms, entry at <=1200ms)
     * Can be called by a real tick falling in the window, or by our predictive timer.
     */
    async executePreemptiveLogic(timeToClose) {
        if (this.waitingForLimitFill || this._preemptiveEntryDone || this._isEntering || this.isClosing) return;
        ...
        const isInstantEntry = this.instantEntry && this.strategy === 'SPIRIT_TEST' && signal && !isTrigger;

        // Manual trigger: detect state transition by comparing with previous candle
        const manualTrigger = !isTrigger && signal !== 'Neutral' && prevCandle &&
            prevCandle.state !== undefined && candle.state !== undefined &&
            candle.state !== prevCandle.state;

        // DEBUG: Log preemptive signal check
        /* Desactivamos este log constante para no asustar en la UI cada milisegundo. Solo lo mostramos cuando el timer lo reporta o entra
        this.log('INFO', `🔍 T-${timeToClose}ms | signal=${signal} state=${candle.state} prevState=${prevCandle?.state} isTrigger=${isTrigger} manualTrigger=${manualTrigger} reEntry=${!!isSpiritReEntry} instant=${!!isInstantEntry}`);
        */

        if (isTrigger || isSpiritReEntry || isInstantEntry || manualTrigger) {
            ...
            // 1. REVERSAL LOGIC (Priority) - Window: T <= 1500ms
            if (this.position && this.position.type !== type && !this.isClosing && timeToClose <= 2000) {
                this.log('INFO', `⚡ Reversión anticipada (T-${timeToClose}ms): Cerrando ${this.position.type} por señal ${type}`);
                try {
                    await this.closePosition(candle.close, 'Signal Reversal (Preemptive)');
                    // Position is now null
                } catch (err) {
                    this.log('ERROR', `Error closing for reversal: ${err.message}`);
                }
            }

            // 2. ENTRY LOGIC - Window: T <= 1200ms
            if (canEnter && !this.position && timeToClose <= 1300 && !this._preemptiveEntryDone) {
                const targetPrice = (this.pendingReEntry && this.pendingReEntry.entryPrice) || candle.close;
                const triggerSource = isTrigger ? 'isTrigger' : (manualTrigger ? 'manualTrigger' : (isSpiritReEntry ? 'reEntry' : 'instant'));

                this._isEntering = true;
                if (isInstantEntry) this.instantEntry = false;

                this.log('INFO', `⚡ Entrada anticipada (${triggerSource}): ${type} MARKET ${timeToClose}ms antes del cierre @ ${candle.close}`);
                try {
                    await this.openPosition(type, targetPrice, candle.time);
                } finally {
                    this._isEntering = false;
                }
                this._preemptiveEntryDone = true;
                this._preemptiveEntryDone = true;
                if (isSpiritReEntry) this.pendingReEntry = null;
            }
        }
    }
    ...
    async processSignal() {
        // SPIRIT_TEST Independent Mode: allow signal reversal to close positions
        // This is the safety net — if trailing never activates, signal reversal closes the position
        if (this.independentMode) {
            const candle = this.candles[this.candles.length - 1];
            const signal = candle.signalStr;
            const isTrigger = candle.isTrigger;

            if (this.position && isTrigger) {
                const oppositeSignal = this.position.type === 'LONG' ? 'Sell' : 'Buy';
                if (signal === oppositeSignal) {
                    this.log('INFO', `🔄 Signal Reversal: ${this.position.type} → ${signal} @ ${candle.close}`);
                    await this.closePosition(candle.close, 'Signal Reversal');
                    // Fall-through logic will handle re-entry if needed
                }
            }

            // Si el bloque preemptive (T-1.2s) falló por lag de red o no había tick, 
            // intentamos entrar de todos modos ahora que la vela cerró oficialmente.
            if (!this.position) {
                const isInstantEntry = this.instantEntry && this.strategy === 'SPIRIT_TEST' && signal && !isTrigger;

                if (isTrigger || isInstantEntry) {
                    if (isInstantEntry) {
                        this.instantEntry = false;
                        this.log('INFO', `⚡ Entrada Inmediata (Fallback): usando señal actual (${signal}) para entrar al cierre`);
                    } else {
                        // Default fallback entry for missed preemptive window
                        this.log('INFO', `⚡ Entrada Fallback (Cierre de vela): preemptive entry no ejecutado para señal ${signal}`);
                    }
                    ...
                    if (canEnter) {
                        await this.openPosition(type, candle.close, candle.time);
                    }
                }
            }
            return;
    ...
    async manageOpenPosition(tick) {
        ...
        // ============================================================
        if (this.strategy === 'SPIRIT_TEST' && this.independentMode) {
            // ============================================================
            // OWN TRAILING STOP: tick-by-tick tracking with MARKET close
            // Binance trailing is unreliable on low-liquidity pairs
            // We track highest/lowest ourselves and close with MARKET
            // Binance 1% trailing placed as safety backup only
            // ============================================================
            if (this.isClosing || tick.time <= p.entryCandleTime) {
                this._trailingDebug = null;
                return; // Skip entry candle & if closing
            }

            // ============================================================
            // FIXED STOP LOSS TIER (Safety Net)
            // Executes a market close immediately if price breaches fixed limit
            // ============================================================
            if (this.useFixedSL) {
                const fixedSlLevel = p.type === 'LONG'
                    ? p.entryPrice * (1 - slRate)
                    : p.entryPrice * (1 + slRate);

                const stopHit = p.type === 'LONG'
                    ? tick.close <= fixedSlLevel
                    : tick.close >= fixedSlLevel;

                if (stopHit) {
                    this.log('INFO', `⛔ Stop Loss Fijo alcanzado: ${p.type} limit=${fixedSlLevel.toFixed(6)} precio=${tick.close.toFixed(6)}`);
                    await this.closePosition(fixedSlLevel, 'Stop Loss Fijo');
                    return;
                }
            }

            // ============================================================
            // TRAILING STOP TIER (Profit Seeker)
            // ============================================================
            if (!this.useTrailing) {
                // Si solo tenemos Stop Fijo, saltamos el trailing
                return;
            }

            const callbackRate = Math.max(0.1, Math.min(this.trailingPct, 5));
            const callbackFraction = callbackRate / 100;

            // Update extremes on EVERY tick
            if (p.type === 'LONG') {
                p.highestPrice = Math.max(p.highestPrice || p.entryPrice, tick.high);
            } else {
                p.lowestPrice = Math.min(p.lowestPrice || p.entryPrice, tick.low);
            }

            // Calculate trailing stop level
            let stopLevel;
            if (p.type === 'LONG') {
                stopLevel = p.highestPrice * (1 - callbackFraction);
                stopLevel = Math.max(stopLevel, p.entryPrice); // FLOOR: never below entry
            } else {
                stopLevel = p.lowestPrice * (1 + callbackFraction);
                stopLevel = Math.min(stopLevel, p.entryPrice); // CEILING: never above entry
            }

            // Check if activation target reached (price moved enough in our favor)
            const minActivation = Math.max(this.trailingActivation || 0, callbackRate);
            const activationFraction = minActivation / 100;
            const activationReached = p.type === 'LONG'
                ? p.highestPrice >= p.entryPrice * (1 + activationFraction)
                : p.lowestPrice <= p.entryPrice * (1 - activationFraction);

            // Check if stop hit (only after activation)
            if (activationReached) {
                if (this.useNativeTrailing) {
                    if (!this._trailingStopPlaced) {
                        this.log('SUCCESS', `🎯 Trailing Activado. Colocando orden nativa en Binance (${callbackRate}%)...`);
                        await this.placeTrailingStopNative(callbackRate);
                        this._trailingStopPlaced = true;
                    }
                    // Una vez colocado nativamente, dejamos que el evento de llenado de Binance (handleStopMarketFilled) cierre la posición.
                } else {
                    const stopHit = p.type === 'LONG'
                        ? tick.close <= stopLevel
                        : tick.close >= stopLevel;

                    if (stopHit) {
                        this.log('INFO', `🛑 Trailing propio: ${p.type} highest=${p.highestPrice?.toFixed(6)} stop=${stopLevel.toFixed(6)} precio=${tick.close.toFixed(6)}`);
                        await this.closeByOwnTrailing(stopLevel);
                        return;
                    }
                }
            }

            // Store debug data for UI
            this._trailingDebug = {
                ...
                stopLevel: stopLevel,
                activationTarget: p.type === 'LONG'
                    ? p.entryPrice * (1 + activationFraction)
                    : p.entryPrice * (1 - activationFraction),
    ...
    async openPosition(type, price, candleTime) {
        ...
        try {
            ...
            // ============================================
            // NATIVE FIXED STOP LOSS PLACEMENT
            // Immediately secure the trade with Binance
            // ============================================
            if (this.useFixedSL && this.stopLossPct > 0) {
                const slRate = this.stopLossPct / 100;
                const fixedSlLevel = type === 'LONG'
                    ? fillPrice * (1 - slRate)
                    : fillPrice * (1 + slRate);

                this.log('INFO', `🛡️ Colocando Fixed Stop Loss inicial en Binance: ${fixedSlLevel.toFixed(5)}`);
                await this.placeStopMarket(fixedSlLevel, fillPrice);
            }
    ...
    async closePosition(price, reason) {
        ...
        try {
            ...
            // If Binance has no position, clear local state to prevent zombie
            if (err.message && err.message.includes('ReduceOnly')) {
                this.log('WARN', 'Binance has no position. Clearing local state to prevent zombie.');
                this.position = null;
                this._trailingStopPlaced = false;
                this._trailingDebug = null;
                this.activeStopOrder = null;
    ...
    async placeStopMarket(stopPrice, currentPrice) {
        ...
        try {
            ...
            const order = await this.client.futuresOrder({
                ...
                quantity: quantity,
                reduceOnly: true,
                workingType: 'CONTRACT_PRICE' // Changed from MARK_PRICE to CONTRACT_PRICE to align with chart candles
            });
            ...
            if (!order.orderId) {
                this.log('WARN', `${typeLabel} placed but no orderId in response — will not be cancellable`);
                this.activeStopOrder = null; // Don't keep an uncancellable reference
            } else {
    ...
    async cancelStopMarket() {
        ...
        // Guard: if orderId is undefined (e.g. from a failed placeStopMarket), just clear the reference
        if (!this.activeStopOrder.orderId) {
            this.activeStopOrder = null;
            return;
        }
    ...
    /**
     * Place Binance native TRAILING_STOP_MARKET order
     * Used as safety backup for own trailing implementation
     * @param {number} overrideRate - Optional callback rate override (default: this.trailingPct)
     */
    async placeTrailingStopNative(overrideRate) {
        if (!this.position) return;
        ...
        try {
            const side = this.position.type === 'LONG' ? 'SELL' : 'BUY';
            const quantity = this.position.size;
            // Binance callbackRate: 0.1 = 0.1%, range 0.1-5
            const callbackRate = Math.max(0.1, Math.min(overrideRate || this.trailingPct, 5));

            // NOTE: activationPrice is NOT passed to Binance — we control activation timing
            // in manageOpenPosition() by waiting to place the order until price reaches target.
            // Binance algo orders ignore/mishandle the activationPrice parameter.

            this.log('INFO', `\ud83d\udccc Colocando TRAILING_STOP_MARKET: callbackRate=${callbackRate}%...`);
            ...
            const order = await this.client.futuresOrder({
                ...
                callbackRate: callbackRate,
                reduceOnly: true,
                workingType: 'CONTRACT_PRICE' // CRUCIAL: Trail the Last Price (Chart), not the lagging Mark Price
            });

            // Debug: log full response to identify correct orderId field
            this.log('INFO', `TRAILING response: ${JSON.stringify(order)}`);

            // Try multiple possible orderId fields from the library
            const resolvedOrderId = order.orderId || order.clientOrderId || order.orderListId || null;
            ...
            if (!resolvedOrderId) {
                this.log('WARN', `TRAILING_STOP_MARKET colocado pero sin orderId — se rastreará por tipo`);
            } else {
                this.log('SUCCESS', `\u2713 TRAILING_STOP_MARKET colocado: ID ${resolvedOrderId}, callbackRate=${callbackRate}%`);
            }

        } catch (err) {
            this.log('ERROR', `\u2717 Error colocando TRAILING_STOP_MARKET: ${err.message}`);
        }
    }

    /**
     * Close position by own trailing stop logic — sends MARKET close order
     * Models after handleStopMarketFilled for proper trade recording & re-entries
     */
    async closeByOwnTrailing(stopLevel) {
        if (!this.position || this.isClosing) return;
        this.isClosing = true;
        ...
        try {
            // Cancel Binance backup trailing first
            await this.cancelStopMarket();
            ...
            const fillPrice = parseFloat(order.avgPrice) || stopLevel;
            const exitCommission = p.size * fillPrice * (this.takerFee || 0.0004);
            ...
            // Calculate PnL
            const pnlRaw = p.type === 'LONG'
                ? (fillPrice - p.entryPrice) * p.size
                : (p.entryPrice - fillPrice) * p.size;
            const netPnL = pnlRaw - exitCommission;

            const reason = 'Trailing Stop';
            const trade = {
                ...p,
                exitPrice: fillPrice,
            ...
            const decimals = fillPrice < 1 ? 6 : 4;
            this.log('SUCCESS', `✓ Trailing propio cerró ${prevType} @ ${fillPrice.toFixed(decimals)} | PnL: ${netPnL >= 0 ? '+' : ''}${netPnL.toFixed(4)} USDT`);

            // Sync balance
            await this.syncAccountState();

            // Re-entry logic
            this.handleSpiritReEntry(reason, prevType, fillPrice);
            this.onStateChange();

        } catch (err) {
            this.log('ERROR', `Error cerrando por trailing propio: ${err.message}`);
            // Binance backup trailing (1%) still active as safety net
        } finally {
    ...
    async handleStopMarketFilled(orderData) {
        ...
        const typeLabel = this.activeStopOrder.orderType === 'TAKE_PROFIT_MARKET' ? 'TP_MARKET' :
            (this.activeStopOrder.orderType === 'TRAILING_STOP_MARKET' ? 'TRAILING_STOP' : 'STOP_MARKET');
    ...
    async placeLimitOrder(type, price, candleTime, initialSL = 0, isElite = false) { // Added isElite parameter
        ...
        try {
            ...
            // Pre-set waitingForLimitFill BEFORE the API call to prevent race condition
            // with User Data Stream (FILLED can arrive before await returns)
            this.waitingForLimitFill = {
                orderId: null, // Will be set after API response
                type: type,
                price: roundedPrice,
                initialSL: initialSL,
                quantity: quantity,
            ...
            // Update with real orderId
            if (this.waitingForLimitFill) {
                this.waitingForLimitFill.orderId = order.orderId;
            }
            this.log('SUCCESS', `→ Orden LIMIT colocada. ID: ${order.orderId}`);

            // Handle instant fill: if order already FILLED in the REST response,
            // process it immediately (User Data Stream might not fire in time)
            if (order.status === 'FILLED') {
                this.log('INFO', `⚡ LIMIT llenado instantáneamente en respuesta REST`);
                await this.onOrderFilled({
                    orderId: order.orderId,
            ...
            this.waitingForLimitFill = null; // Clean up on error
        }
    }
        ...
        try {
            ...
            if (order.status === 'FILLED') {
                ...
                if (this.useFixedSL && this.stopLossPct > 0) {
                    ...
                    this.log('INFO', `🛡️ [LIMIT FILL] Colocando Fixed Stop Loss en Binance: ${fixedSlLevel.toFixed(5)}`);
                    await this.placeStopMarket(fixedSlLevel, fillPrice);
                }
    ...
    async cancelPendingOrder(reason) {
        if (!this.waitingForLimitFill) return;
        const pendingOrder = this.waitingForLimitFill;
        this.log('INFO', `Cancelling Pending Order ${pendingOrder.orderId} due to: ${reason}`);
        ...
        try {
            ...
            // Check for partial fill — if some qty was filled, create a position
            const filledQty = parseFloat(result.executedQty || 0);
            if (filledQty > 0) {
                const fillPrice = parseFloat(result.avgPrice || result.price || pendingOrder.price);
                const filledRatio = filledQty / pendingOrder.quantity;
                this.log('WARN', `⚠️ Partial fill detectado: ${filledQty} (${(filledRatio * 100).toFixed(1)}%) llenado @ ${fillPrice}`);
                this.log('INFO', `Creando posición parcial — se cerrará normalmente`);

                this.position = {
                    type: pendingOrder.type,
                    entryPrice: fillPrice,
                    size: filledQty,
                    margin: pendingOrder.margin * filledRatio,
                    entryTime: pendingOrder.candleTime,
                    entryCandleTime: pendingOrder.candleTime,
                    highestPrice: fillPrice,
                    lowestPrice: fillPrice,
                    initialSL: pendingOrder.initialSL || 0,
                    candlesSinceEntry: 0,
                    lastCandleTime: pendingOrder.candleTime,
                    entryCommission: 0,
                    entryOrderId: pendingOrder.orderId,
                    accumulatedFunding: 0,
                    lastFundingCheck: pendingOrder.candleTime,
                    isElite: pendingOrder.isElite || false,
                    experimentalSL: null,
                    experimentalSLSet: false,
                    lastReportedSLType: null
                };
    ...
    async onOrderFilled(orderData) {
        ...
        // Allow match if orderId is null (race condition: fill arrived before API response)
        if (orderId !== null && orderData.orderId !== orderId) {
            this.log('WARN', `onOrderFilled: OrderId mismatch. Expected ${orderId}, got ${orderData.orderId}`);
            return;
        }

        // If orderId was null, update it now
        if (orderId === null) {
            this.waitingForLimitFill.orderId = orderData.orderId;
        }
    ...
    handleSpiritReEntry(reason, lastType, exitPrice) { // SPIRIT RE-ENTRY Setup
        // SPIRIT_TEST: handle own re-entries (trailing stop close → wait for same signal)
        // Other strategies: only if not in mirror mode
        if (!this.independentMode && this.paperBotRef) return; // Non-independent mirror → paper handles re-entry
    ...
    getStatus() {
        return {
            ...
            pendingVirtualOrder: this.pendingVirtualOrder,
            ...
            trailingDebug: this._trailingDebug || null
        };
    }
}
