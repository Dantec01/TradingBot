...
class TradingEngine {
    ...
    linkToPaperBot(paperEngine) {
        ...
        // SPIRIT_TEST independent: skip if real already has position or trailing active
        paperEngine.events.on('paper:open', async (data) => {
            ...
            // SPIRIT_TEST independent: preemptive entry handles entries, skip paper:open
            if (this.preemptiveCloseEnabled) {
                this.log('INFO', `🪞 Paper abrió ${data.type} → Modo independiente, entrada por bloque anticipado`);
                return;
            }
    ...
    async update(newCandle) {
        ...
        // SPIRIT_TEST + TRAILING + trailingPct <= 0.5%
        // Opens MARKET using same indicators as paper bot
        ...
        if (this.preemptiveCloseEnabled && !this.isClosing && newCandle.eventTime && newCandle.closeTime) {
            ...
            if (timeToClose > 0 && timeToClose <= 1000) {
                ...
                if (!this.position && !this.waitingForLimitFill && !this._preemptiveEntryDone) {
                    ...
                    // Manual trigger: current candle has signal but isTrigger is false →
                    // compare with previous candle's state to detect transition
                    ...
                    this.log('INFO', `🔍 PRE-ENTRY CHECK ${timeToClose}ms | signal=${signal} state=${candle.state} prevState=${prevCandle?.state} | isTrigger=${isTrigger} manualTrigger=${manualTrigger} reEntry=${!!isSpiritReEntry} instant=${!!isInstantEntry} | pos=${!!this.position} done=${this._preemptiveEntryDone}`);
