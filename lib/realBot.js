...
class TradingEngine {
    constructor(config) {
        ...
        // SPIRIT_TEST Independent Mode: preemptive entry + Binance native trailing
        // Active for SPIRIT_TEST + TRAILING (any percentage)
        this.preemptiveCloseEnabled = this.mirrorMode && this.slMode === 'TRAILING';
        this._trailingStopPlaced = false; // Track if trailing was placed for current position
    ...
    linkToPaperBot(paperEngine) {
        ...
        // When paper opens a position → enter with MARKET for SPIRIT_TEST independent mode
        // Uses paper's reliable signals but executes immediately with MARKET (not LIMIT)
        paperEngine.events.on('paper:open', async (data) => {
            if (data.symbol !== this.symbol.replace('USDT', '')) return;

            // If we already have a position, pending order, active trailing, or entering → skip
            ...
            // SPIRIT_TEST independent: use paper's signal, enter with MARKET immediately
            if (this.preemptiveCloseEnabled) {
                const type = data.type; // 'LONG' or 'SHORT'
                const canEnter = (type === 'LONG' && (this.direction === 'BOTH' || this.direction === 'LONG')) ||
                    (type === 'SHORT' && (this.direction === 'BOTH' || this.direction === 'SHORT'));

                if (canEnter) {
                    this._isEntering = true; // Lock to prevent double entry
                    this.log('INFO', `⚡ Paper abrió ${type} → Entrando con MARKET @ mercado`);
                    try {
                        await this.openPosition(type, data.price, data.candleTime);
                    } finally {
                        this._isEntering = false;
                    }
                } else {
                    this.log('INFO', `🪞 Paper abrió ${type} → Bloqueado por dirección=${this.direction}`);
                }
                return;
            }
        ...
        // SPIRIT_TEST independent: skip SL updates (Binance trailing handles it)
        paperEngine.events.on('paper:sl_update', async (data) => {
            if (data.symbol !== this.symbol.replace('USDT', '')) return;

            // SPIRIT_TEST independent: Binance native trailing handles SL
            if (this.preemptiveCloseEnabled) return;
        ...
        // When paper closes → only act on Signal Reversals for independent mode
        paperEngine.events.on('paper:close', async (data) => {
            if (data.symbol !== this.symbol.replace('USDT', '')) return;

            // SPIRIT_TEST independent: only close on signal reversals, NOT trailing
            if (this.preemptiveCloseEnabled) {
                if (data.reason === 'Signal Reversal' && this.position) {
                    this.log('INFO', `🪞 Paper cerró por REVERSAL → Cerrando real`);
                    await this.closePosition(data.price, 'Signal Reversal');
                } else {
                    this.log('INFO', `🪞 Paper cerró (${data.reason}) → Ignorado, trailing nativo activo`);
                }
                return;
            }
    ...
    async update(newCandle) {
        ...
        // ============================================================
        // PREEMPTIVE ENTRY: 1s before candle close (Binance time)
        // SPIRIT_TEST + TRAILING: Opens MARKET using own indicators
        // Both paper and real load 500 candles → indicators should match
        // Exit handled by Binance native TRAILING_STOP_MARKET
        // ============================================================
        if (this.preemptiveCloseEnabled && !this.isClosing && !this._isEntering && newCandle.eventTime && newCandle.closeTime) {
            const timeToClose = newCandle.closeTime - newCandle.eventTime;
            if (timeToClose > 0 && timeToClose <= 1200) {
                // --- PREEMPTIVE ENTRY ---
                if (!this.position && !this.waitingForLimitFill && !this._preemptiveEntryDone) {
    ...
    async manageOpenPosition(tick) {
        ...
        if (this.strategy === 'SPIRIT_TEST' && this.mirrorMode) {
            ...
            // ============================================================
            if (this.isClosing || tick.time <= p.entryCandleTime) return; // Skip entry candle & if closing
            ...
            if (activationReached) {
                ...
                // Place Binance backup trailing (1%) ONLY after activation — so backup also never closes in negative
                if (tick.isFinal && !this._trailingStopPlaced) {
                    await this.placeTrailingStopNative(1.0); // 1% wide backup
    ...
    handleSpiritReEntry(reason, lastType, exitPrice) { // SPIRIT RE-ENTRY Setup
        // Mirror Mode (non-independent): Paper bot handles re-entry, not us
        // But in independent mode (preemptiveCloseEnabled), we handle our own re-entries
        if (this.mirrorMode && !this.preemptiveCloseEnabled) return;
