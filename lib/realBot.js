...
class TradingEngine {
    ...
    async manageOpenPosition(tick) {
        ...
        // Uses STOP_MARKET on Binance, updated on every tick for precision
        // ============================================================
        if (this.strategy === 'SPIRIT_TEST' && this.slMode === 'TRAILING') {
            ...
            // NO STOP_MARKET on Binance — monitor locally and close with MARKET
            // This matches paper bot behavior exactly and avoids desynchronization
            // where Binance's STOP_MARKET triggers before the trailing has time to update
            if (tick.time > p.entryCandleTime && p.trailingSLLevel > 0) {
                ...
                if (hit) {
                    ...
                    this.log('INFO', `🎯 RT Trailing hit @ ${p.trailingSLLevel.toFixed(decimals)} — cerrando con MARKET`);
