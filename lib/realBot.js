...
class TradingEngine {
            ...
            if (isTrigger || isSpiritReEntry) {
                ...
                if (canEnter) {
                    ...
                    if (this.strategy === 'BACKGUARD') {
                        ...
                        this.log('INFO', `🎯 SPIRIT_ELITE - Orden VIRTUAL colocada: ${type} @ ${limitPrice.toFixed(5)} (esperando condición de fill)`);
                    } else {
    ...
    async manageOpenPosition(tick) {
        ...
        if (this.strategy === 'SPIRIT_ELITE') {
            ...
            // Debug log when SL type changes
            if (p.lastReportedSLType !== usedSLType && usedSLType !== 'None') {
                this.log('INFO', `[SPIRIT_ELITE] SL Active: ${usedSLType} @ ${finalSL.toFixed(4)} | Candles: ${p.candlesSinceEntry} | Entry: ${p.entryPrice.toFixed(4)}`);
    ...
    async openPosition(type, price, candleTime) {
        ...
        try {
            ...
            this.log('SUCCESS', `✓ ORDEN MARKET ejecutada: ID ${order.orderId}, Precio ${order.avgPrice || price}`);
    ...
    async closePosition(price, reason) {
        ...
        try {
            ...
            this.log('SUCCESS', `✓ POSICIÓN CERRADA: ID ${order.orderId}, Precio ${order.avgPrice || price}`);
    ...
    async checkPendingVirtualOrder(tick) {
        ...
        if (filled) {
            this.log('SUCCESS', `✓ SPIRIT_ELITE - Orden VIRTUAL activada! ${type} @ tick.close=${tick.close.toFixed(5)}`);
