...
class BotManager {
    ...
    async loadState() {
        ...
        try {
            ...
            for (const botData of data.bots) {
                ...
                try {
                    const symbol = botData.config.symbol.toUpperCase();
                    const config = {
                        ...botData.config,
    ...
    clearHistory() {
        try {
            ...
            console.log("[Bot Manager] History cleared.");
            return true;
        } catch (err) {
            console.error("[Bot Manager] Error clearing history:", err);
            return false;
        }
    }
    ...
    updateSubscription() {
        ...
        for (const [symbol, engine] of this.instances) {
            ...
            // VANGUARD Auxiliary 1m Timeframe
            ...
            if (msg.e === 'kline') {
                ...
                if (engine) {
                    ...
                    const candle = {
                        ...
                        isFinal: k.x,
                        interval: k.i // Pass interval to help debugging or routing
                    };

                    // Route based on interval
                    if (k.i === engine.timeframe) {
                        engine.update(candle);
                    } else if (k.i === '1m' && engine.strategy === 'VANGUARD') {
                        if (engine.processAuxiliaryCandle) {
                            engine.processAuxiliaryCandle(candle);
                        }
                    }
                }
            }
